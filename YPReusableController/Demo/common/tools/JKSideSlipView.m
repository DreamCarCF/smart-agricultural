...
-(void)buildViews:(UIViewController*)sender{
    ...
    //设置模糊图片
    _blurImageView = [[UIImageView alloc] initWithFrame:CGRectMake(-SLIP_WIDTH, 0, SLIP_WIDTH, [UIScreen mainScreen].bounds.size.height)];
...
-(void)show:(BOOL)show{
    ...
    if (!isOpen) { //isOpen = false, blueImageview.alpha = 1
        _blurImageView.alpha = 1;
    }
    
    CGFloat x = show ? 80 : -SLIP_WIDTH;
    [UIView animateWithDuration:0.3 animations:^{
        self.frame = CGRectMake(x, 0, self.frame.size.width, self.frame.size.height);
        if(!isOpen){
            _blurImageView.image = image;
            _blurImageView.image= [self blurryImage:_blurImageView.image withBlurLevel:0.3];
        }
